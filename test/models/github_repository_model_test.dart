...
void main() async {
  test('deve retornar os repositorios', () async {
    final client = IHttpClientMock();

    when(() => client.get(any()))
        .thenAnswer((_) async => jsonDecode(jsonResponse));
    final service = JsonGitHubService(client);
    final repositories = await service.getRepository();
    print(repositories[0].name);
    expect(repositories[0].name, 'grit');
  });
}
